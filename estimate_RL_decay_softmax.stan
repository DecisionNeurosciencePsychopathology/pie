...
transformed parameters {
  ...
  lambda=lambda_m + lambda_s*lambda_raw; //rescale so params are approx N(0,1)
  ...
  for (s in 1:nS) {
    for (t in 1:nT) {
      ...
      for (j in 1:num_segments[s,t]) {
        Q[s,t+1,j] = choice[s,t]==j ? (Q[s,t,j] + alpha[s]*(reward[s,t]-Q[s,t,j])) : lambda[s]*(Q[s,t,j]-0.5);
      }
...
generated quantities {
  ...
  for (s in 1:nS) {
    for (t in 1:nT) {
      ...
      for (j in 1:num_segments[s,t]) {
        Q[j] = choice[s,t]==j ? (Q[j] + alpha[s]*(reward[s,t]-Q[j])) : lambda[s]*(Q[j]-0.5);
      }
