...
transformed parameters {
  ...
  vector[nS] beta;
  vector[nS] eps_pre;
  vector<lower=0,upper=1>[nS] eps;
  
  alpha_pre=alpha_m + alpha_s*alpha_raw;
  alpha=inv_logit(alpha_pre);
  eps_pre=eps_m + eps_s*eps_raw;
  eps=inv_logit(eps_pre);
  ...
  for (s in 1:nS) {
    for (t in 1:nT) {
      ...
      if(t>num_segments[s,t]&&(block_num[s,t]-block_num[s,t-num_segments[s,t]+1]==0)) {
        choice[s,t] ~ categorical_logit((1-eps[s])*(beta[s]*Q[1:num_segments[s,t]]) + eps[s]/num_segments[s,t]);
      }
...
generated quantities {
  ...
  for (s in 1:nS) {
    for (t in 1:nT) {
      ...
      // (note: this assumes # of forced choice trials = # of segments)
      if(t>num_segments[s,t]&&(block_num[s,t]-block_num[s,t-num_segments[s,t]+1]==0)) {
        log_lik[s,t] = categorical_logit_lpmf(choice[s,t]|(1-eps[s])*(beta[s]*Q[1:num_segments[s,t]]) + 
          eps[s]/num_segments[s,t]);
      } else {
